...
# === HEADER ===
print(Z + "□■" * 30)
ascii_art = pyfiglet.figlet_format("   Satan ")
print(L + ascii_art)
print('\t\x1b[38;5;153m PayPal Charge 1$')
print(F + "□■" * 30)

# === STATIC OWNER BOT ===
combo = input(X + 'CHEAKERONTOP: ')
try:
    with open(combo, "r") as y:
        lines = y.readlines()
except Exception as e:
    print(Z + f"Failed to open file: {e}")
    exit()

# === TOKEN & OWNER ===
tokbot = '7964928255:AAFOsLr9zDbLQXoZMxrhFzd54uJEzgd33QE'  # Replace with your actual bot token
idbot = 930577300  # Owner ID
...
for line in lines:
    ...
    if len(parts) < 4:
        print(Z + f"Invalid CC format: {card}")
        continue
    n, mm, yy, cvc = parts[:4]

    # Add your checker logic here...

    time.sleep(5)  # 5-second delay per CC
